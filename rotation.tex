...
First we show that it is sufficient to estimate total curvature for the minimizing geodesics with almost constant velocity vector, say $\dot\gamma(t)\approx\bm{k}$.

To estimate total curvature in this case it is sufficient 
to estimate the integral
\[\int\langle\ddot\gamma(t),\bm{i}\rangle\cdot dt\] for suitable vector $\bm{i}\perp \bm{k}$.
To understand the idea of this estimate, 
imagine that 
the surface is lighten in the direction of $\bm{i}$,
...
In the first case the total curvature is estimated by integral of Gauss curvature of the regions squeezed between $\gamma$ and $\omega$; see Tongue Lemma \ref{lem:tongue}.

The second case might look impossible, 
but the corkscrew geodesic constructed in \cite{BKZ} can intersect $\omega$ this way.
...
is called \emph{elevating} if both ends $\gamma(0)$ and $\gamma(\ell)$ lie on the $z$-axis and 
...
Assume $\gamma\:[0,\ell]\to \Sigma$ is an elevating minimizing geodesic in the convex surface $\Sigma\subset \RR^3$ and a sub-segment $[a,b]\subset [0,\ell]$ the following conditions hold
\begin{enumerate}[(i)]
...
lies on the dark side of $\Sigma$ with respect to $\bm{k}$.
...
Let $t_0<t_1<\dots<t_k$ be the meeting moments of $\gamma$ with $\omega_{\bm{i}}$
Set
\begin{align*}
\phi_n&=\phi(t_n)
&
...
if $\nu_{\gamma(t)}$ moves west in $\SS^2$
at $t_n$
and $s_n=-1$ if it moves east.    
...
Then the arcs $\gamma|_{[t_i,t_j]}$ and an arc of $\omega_{\bm{i}}$ bound an immersed disc in $\Sigma$ which lies between horizontal planes through $\gamma(t_i)$ and $\gamma(t_j)$.
...
\parit{Proof.}
We can assume that
$\gamma$ cross the $\bm{i}$-horizon $\omega_{\bm{i}}$ transversely.
Let $t_0<\dots<t_k$ be the meeting moments of $\gamma$ with $\omega_{\bm{i}}$ and
...
we can pass to an elevating arc $\gamma'$
...
above the horizontal line through $\tfrac12\cdot(\gamma'(0)+\gamma'(\ell))$ which rotates around $z$-axis 2 times and $b$ takes the minimal possible value.  
...
The remaining arc is assumed to be $\gamma_0'$; note that any sub-arc of $\gamma_0'$ is divided by the horizontal plane through $\tfrac12\cdot(\gamma'(0)+\gamma'(\ell))$ into two each of which rotate around $z$-axis at most 2 times. 
...
\[\tc_{\bm{i}}\gamma'_+\le 100\cdot\pi.\eqlbl{+}\]
Similarly  
\[\tc_{\bm{i}}\gamma'_-\le 100\cdot\pi.\eqlbl{-}\]

By Corollary~\ref{cor:gamma-0},
\[\tc_{\bm{i}}\gamma'_0\le 100\cdot\pi.\eqlbl{0}\]
