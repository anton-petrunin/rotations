...
The existence of a universal upper bound on total curvature of minimizing geodesic on convex surface was conjectured by Dmitry Burago;
it was also stated as an open question in \cite{AH-PSV}, \cite{pach} and \cite{BKZ}.
...
and the segment $[\tilde_{\bm{u}}\gamma(a)\tilde\gamma_{\bm{u}}(b)]$
...
If $\Sigma$ is strongly convex then $\omega_u$ is locally a smooth curve.
...
where the sign is ``$+$'' 
if $[t_i,t_{i+1}]$ lies on the dark side 
...
\begin{thm}{Lemma}
Let $\gamma$ be a geodesic on the strongly convex surface $\Sigma$.
Assume that the ends of $\gamma$ 
lies on the horizon $\omega_{\bm{u}}$
of $\bm{u}$
and there an open map $\iota\:\DD\to\Sigma$
such that the image of the boundary $\partial\DD$
is formed by $\gamma$ and an arc of $\omega_{\bm{u}}$.
Then 
...
Recall that $\omega_{\bm{u}}$ denotes the horizon of the unite vector $\bm{u}$ on $\Sigma$;
it forms by a collection of disjoint smooth curves.

Assume $\Sigma$ is a smooth strongly convex surface in $\RR^3$
...
Since $\tc_{\bm{u}}\gamma=\alpha-\beta$,
we get
