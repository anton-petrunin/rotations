...
that any the spherical image of geodesic on convex surface has to be contructable.
...
for any $t\in [0,\ell]$
and (3) the projection of $\tilde\gamma_{\bm{u}}(t)$ to the line normal to $\tilde{\bm{u}}$
is monotonic in $t$.
...
If in addition $\Sigma$ is closed strongly convex then $\omega_{\bm{u}}$ is locally a smooth curve.
...
Since $\alpha_1,\alpha_k\le\tfrac\pi2$, the statement follows.
\qeds 
...
Assume $t_i,t_j$ be the meeting moments of minimizing geodesic $\gamma$ with $\omega_{\bm{u}}$
The arc $\gamma|_{[t_i,t_j]}$ will be called \emph{$\omega_{\bm{u}}$-tongue}
if there is an immersed disc 
$\iota\:\DD\looparrowright\Sigma$ 
such that the closed curve $\iota|_{\partial D}$ is formed by joint of the arc $\gamma|_{[t_i,t_j]}$ and an arc of $\omega_{\bm{u}}$ bounds an.
...
Then $\int\limits_{\DD} K_{\iota(x)}\cdot d_{\iota(x)}\area_\Sigma$ takes one of the value
\[\alpha-\beta,
\  -\alpha+\beta,
\ \pi-\alpha-\beta,
 \pi+\alpha+\beta \pmod{2\cdot\pi}
\]
if the pair of meeting angles $(s_a,s_b)$ 
...
and explains the difficulty of the general case.
...
Note that each arc $\gamma_{[t_{n-1},t_n]}$ is a $\bm{u}$-tongue.
The discs of these tongues do not overlap and each lies on the 
dark or light side of $\bm{u}$.
...
Then $\diam \gamma\ge \ell/10$.
\end{thm}


\section{Elevating arc}
...
we can pass to an elevating arc, of $\gamma$ for some $(x,y,z)$-coordinate system with total curvature $>\tfrac{\omega}{100^{100}}$%???NUMBER
.
Denote this arc by $\gamma'$ and let us use the notations in Section~\ref{sec:elevating}.
...
we can subdivide $\gamma'$ in 3 arcs $\gamma_-'$, $\gamma_0'$ and $\gamma_+'$
such that $\gamma_-$ lies on the light side for $\bm{k}$,
$\gamma_+$ lies on the dark side for $\bm{k}$
and any arc in 
$\gamma_0$ rotatates around $z$-axis at most $100$ times.
