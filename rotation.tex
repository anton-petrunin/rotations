...
Let $\Sigma$ be a convex surface in $\RR^3$
...
This question was formulated in \cite{AH-PSV}, \cite{pach} and \cite{BKZ},
...
\item{\it (Proposition \ref{prop:almost-const}).}  
The $z$-component of $\dot\gamma$ is positive,
...
On the second step we give number of geometric inequalities which relate the angles between $\dot\gamma(t)$ and with the coordinate axis.
...
\begin{thm}{Claim}
Let $\gamma\:[0,\ell]\to\Sigma$
be elevating minimizing geodesic 
in a strongly convex surface $\Sigma$
and $t_n$, $s_n$, $\phi_n$ as defined above.

Assume there are indexes $k_0<k_1$ such that
we have the point $\gamma(t_{k_0})$ lies above the horizontal plane through $\tfrac12\cdot(\gamma(0)+\gamma(\ell))$
and 
\[|\sum_{n=k_0}^{k_1}s_n|\ge 5.\]
Then for any pair of indexes $j>i>k_1$,
such that 
\[|\sum_{n=i}^{j}s_n|\ge 5\]
we have
\[\phi_j>2\cdot\phi_i.\]
...
\parit{Proof.}
Note that the arc $\gamma|_{[t_{k_0},t_{k_1}]}$ goes around the $z$-axis twice.
By Corollary~\ref{cor:around-twice},
the arc $\gamma|_{t_{k_1},\ell}$ lies in the dark side of $\Sigma$ with respect to $\bm{k}$.
By Claim ???, the function $\phi$ increase in the interval $[t_{k_1},\ell]$.
...
\begin{thm}{Proposition}
There is c constant $\omega'$ ($\omega'=???$ will do)
such that
...
After rotationg $(x,y)$ plane we can assume that 
\[\tc\gamma\le 10\cdot\tc_{\bm{i}}\gamma.\]
Hence the result follows.



\section{Final proof assembling}
...
\[\tc\gamma_+\le 100\cdot\pi.\eqlbl{+}\]
Similarly $\gamma_-$ lies on a graph of convex function and the same proposition implies 
\[\tc\gamma_-\le 100\cdot\pi.\eqlbl{-}\]

It remains to estimate $\tc\gamma_0$.

Note that any $\bm{s}$ pair on $\gamma_0$ has depth at most ???.
Therefore ???

It remains to note that the total number of indexes which do not appear in any $\bm{s}$ on $\gamma_0$ is at most ???.
Hence 
\[\sum_{t_n\in ???}s_n\cdot\alpha_n\le ???\]
By Liberman's lemma, we get
\[\tc_{\bm{i}}\gamma_0\le ???\]
Since we can rotate $(x,y)$-plane arbitrary,
we get
\[\tc\gamma_0\le ???\]
