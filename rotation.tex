...
Then $\int\limits_{\DD} K_{\iota(x)}\cdot d_{\iota(x)}\area_\Sigma$ takes one of the following values
$\alpha-\beta$, $2\cdot\pi-(\alpha-\beta)$, $\pi\pm(\alpha+\beta)$ modulo $2\cdot\pi$
The values are taken depending on the signs $s_a$ and $s_b$...
