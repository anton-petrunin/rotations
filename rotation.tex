...
&\le(N+1)\cdot \pi.
\end{align*}
Therefore if $\gamma$ violetes Main Theorem then it has to cross the  horisons $\omega_{\bm{i}}$, $\omega_{\bm{j}}$ and $\omega_{\bm{k}}$ more that $10^9$ times.%???NUMBER???
...
such that 
\[\tc(\gamma|_{[a,b]}>\tfrac1{10^7}\cdot\tc\gamma.\]
and 
\[\measuredangle(\dot\gamma(t),\bm{u})<\tfrac\pi2-\tfrac1{100}\] 
for any $t\in[a,b]$
and a fixed unit vector $\bm{u}$.
\end{thm}

\parit{Proof.}
By Lemma~\ref{lem:eps-straight}, the geodesic $\gamma$ can be subdivided into twenty $\tfrac1{10}$-straight arcs.
Let us choose the arc $\gamma'$ with the maximal total curvature.
Clearly 
\[\tc\gamma'\ge\tfrac1{20}\cdot\tc\gamma.\]

Let $\alpha_1$ be the angle,
$\bm{u}_1$ be the vector
and $\gamma_1$ be the arc in $\gamma'$ 
provided by Lemma~\ref{lem:almost-const}.

If $\alpha\ge\tfrac\pi2+\tfrac1{10}$ or $\alpha\le \tfrac\pi2+\tfrac2{10}$ then statement follows.
...
denote by $\gamma_1'$ the $\tfrac1{10}$-straight 
arc with maximal total curvature.
Again, we get
\[\tc\gamma_1'\ge\tfrac1{20}\cdot\tc\gamma_1.\]

Further apply Lemma~\ref{lem:almost-const},
denote by $\gamma_2$ and $\alpha_2$ the obtained subarc and angle.
Again, if $\alpha_2\ge\tfrac\pi2+\tfrac1{10}$ or $\alpha_2\le \tfrac\pi2+\tfrac2{10}$ then statement follows.


Otherwise $\gamma''''$ runs in the direction which is nearly perpendicular to the direction to $p$ and $q$.
Namely ???
Since ???
...
Let $\gamma\:[0,\ell]\to\Sigma$ be a $\tfrac1{1000}$-straight minimizing geodesic 
on a smooth strongly convex surface $\Sigma$ in $\RR^3$.
Then there is an interval $[a,b]\subset[0,\ell]$
such that 
\[\tc(\gamma|_{[a,b]})\ge\tfrac1{2000}\cdot\tc\gamma.\]
and 
\[\alpha
\le
\measuredangle(\dot\gamma(t),\gamma(\ell)-\gamma(0))
\le\alpha+\tfrac1{10}\] 
some fixed $\alpha$
and
any $t\in[a,b]$.
...
Set $p=\gamma(0)$.
Since $\gamma$ is $\tfrac1{1000}$-straight,
$|\gamma(t)-p|\ge 1-\tfrac1{1000}$ 
and
\[\measuredangle(\bm{v},\gamma(t)-p)
\le
...
By Liberman's lemma 
\[\tc_p(\gamma|_{[1,2]})\le \tfrac1{20}+\pi<4.\]
It follows that we can subdivide $\gamma|_{[1,2]}$ into $100$ arcs 
such that $\gamma_1,\gamma_2,\dots,\gamma_{100}$ such that
\[\tc_p(\gamma_n)\le \tfrac{1}{20}\]
for each $n$.

It follows that for each $n$, there is $\alpha_n$ such that
\[\alpha_n
\le
\measuredangle(\dot\gamma_n(t),\gamma_n(\ell)-\gamma(0))
\le
\alpha_n+\tfrac1{10}.\] 
