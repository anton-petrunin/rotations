...
\theta_n&=\theta(t_n).
\end{align*}
Note that the definition of $\theta_n=s_n\cdot\alpha_n$ where $s_n$ and $\alpha_n$ are the meeting sign and angle at $t_n$ defined in Section~\ref{sec:curv-develop}.
...
More preisely, the depth of $(i,j)$ is the maximal number $q$
...
Then
\[\left|\sum_{n=1}^k (-1)^n\cdot\theta_n\right|
