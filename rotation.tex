...
\parit{Proof.} For each $n$ denote by $K_n$ the integral of Gauss curvature of the part of surface $\Sigma$ with the $x$-coordinate less then the $x$-coordinate of $\gamma(t_n)$.
Note that 
...
Note that our main theorem does not hold if one removes word \emph{minimizing} from its formulation even if the length of geodesic is bounded in terms of the diameter of the surface.
The minimizing geodesics have distinguishing properties in the class of all geodesics on a convex hypersurfaces.
We want to mention some of them although they did not help us in the proof.
