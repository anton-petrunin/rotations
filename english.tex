...
\renewcommand*{\HyperDestNameFilter}[1]{\jobname-#1}%might help in arXiv

\begin{document}
\title{On the total curvature of \\
minimizing geodesics on convex surfaces}
\author{Nina Lebedeva and Anton Petrunin}
\nofootnote{1991 Mathematics Subject Classification. 53A05, 53C45, 53C22.}
\nofootnote{N.~Lebedeva was partially supported by RFBR grant 14-01-00062.}
...
later we found the same question in \cite{AH-PSV}, \cite{pach} and \cite{BKZ}.
...
this is sufficient to estimate the total curvature of the whole geodesic.
...
\]
where $\alpha$ and $\beta$ are the meeting angles at $a$ and $b$ correspondingly.
...
$\bm{v}_1$ and $\bm{v}_2$  are two vectors in $\RR^3$
...
and $\delta$ is sufficiently small,
...
Consider a plane $L$ 
passing thru two points of $\gamma$, 
say $p=\gamma(a)$ and $q=\gamma(b)$ with $a<b$.
Let $L_\pm$ be a  half-planes in $L$ bounded by the line thru $p$ and $q$.
...
Let $\gamma\:[a,b]\to\RR^3$ be a curve and $\ell$ be a line which does not pass thru points of $\gamma$.
...
If $b\le s<t\le \ell$ and the point $\gamma(s)$ lies in the plane $\Pi$ thru $\gamma(t)$ spanned by $\nu(t)$ and $\lambda(t)$,
then 
\[\phi(s)\le \psi(t).\]
\end{thm}
...
so we can say $s_n\cdot\theta_n=(-1)^n\cdot \alpha_n$ 
...
Then the arcs $\gamma|_{[t_i,t_j]}$ and an arc of $\omega_{\bm{j}}$ bound an immersed disc in $\Sigma$ which lies between $(y,z)$-planes thru $\gamma(t_i)$ and $\gamma(t_j)$.
...
\parit{Proof.} For each $n$ denote by $K_n$ the integral of Gauss curvature of the part of surface $\Sigma$ with the $x$-coordinate less than the $x$-coordinate of $\gamma(t_n)$.
Note that 
...
The remaining arc is assumed to be $\gamma_0'$; note that any sub-arc of $\gamma_0'$ is divided by the $(y,z)$-plane thru $\tfrac12\cdot(\gamma'(0)+\gamma'(\ell))$ into two each of which rotate around $x$-axis at most 2 times. 
...
We want to mention them altho they did not help us in the proof.
